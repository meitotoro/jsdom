...
function highlightPage() {
    ...
    for (var i = 0; i < links.length; i++) {
        ...
        if (window.location.href.toLowerCase().indexOf(linkurl) != -1) {
...
function moveElement(elementID,final_x,final_y,interval)
{
    ...
    if (xpos > final_x) {
        var dist = (xpos - final_x) / 10;
        xpos = xpos - dist;
    }
    if (ypos > final_y) {
        var dist = (ypos - final_y) / 10;
        ypos = ypos - dist;
    }
...
function displayAbbrevations() {
    ...
    for (key in defs) {
        ...
        var ddest_text = document.createTextNode(definition);
        ddesc.appendChild(ddest_text);
